...
protocol HomeBestHeaderDelegate: AnyObject {
    
    func seeMoreBest()
}

class HomeBestHeader: UITableViewHeaderFooterView {
    
    weak var delegate: HomeBestHeaderDelegate?

    static var headerId = "homeBestHeader"
...
private extension HomeBestHeader {
    
    @objc func captionButtonAction() {
        
        delegate?.seeMoreBest()
    }
}
